...
# install required packages for Ubuntu 18.04 (Wiki: Dev Area/Dev Env)
...
sudo apt-get -y install make curl wget xvfb git gitk postgresql
...
#ff might not be necessary if already disabled
a2dismod mpm_worker
a2dismod mpm_event
...
# install npm from here even though bionic has 8.x
# there is a version dependency issue with node-gyp as of 2019-10-30
